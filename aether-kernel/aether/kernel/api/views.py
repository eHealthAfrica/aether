...
class SubmissionStatsMixin(MtViewSetMixin):
    ...
    def get_queryset(self):
        ...
        def _is_included(field_name):
            return (not _fields or field_name in _fields) and (not _omit or field_name not in _omit)
        ...
        if _is_included('entities_count'):
            ...
            if self.request.query_params.get('passthrough', 'false') == 'true':
                ...
                project_id = Cast(f'{self.entities_field}__project', TextField())

                entities_filter = Q(**{
                    f'{self.entities_field}__schema__family': project_id,
                ...
                if family:
                    entities_filter = Q(**{
                        f'{self.entities_field}__schema__family': family,
                    })
            ...
            qs = qs.annotate(entities_count=entities_count)

        if _is_included('first_submission'):
            qs = qs.annotate(first_submission=Min(f'{self.submissions_field}__created'))
        if _is_included('last_submission'):
            qs = qs.annotate(last_submission=Max(f'{self.submissions_field}__created'))
        if _is_included('submissions_count'):
            qs = qs.annotate(submissions_count=Count(f'{self.submissions_field}__id', distinct=True))
        if _is_included('attachments_count'):
            qs = qs.annotate(attachments_count=Count(f'{self.submissions_field}__attachments__id', distinct=True))
