...
class PostSubmissionTests(CustomTestCase):
    ...
    def setUp(self):
        ...
        # create survey in kernel testing server
        response = requests.post(kernel_utils.get_surveys_url(),
                                 json=testing_survey,
                                 headers=self.KERNEL_HEADERS)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED, response.json())
        data = response.json()
        mapping_id = data['id']
        self.SURVEY_URL = kernel_utils.get_surveys_url(mapping_id)
        self.RESPONSES_URL = kernel_utils.get_survey_responses_url(mapping_id)
        # create xForm entry
    ...
    def tearDown(self):
        ...
        requests.delete(self.SURVEY_URL, headers=self.KERNEL_HEADERS)
