...
def __is_leaf(avro_type):
    ...
    return (
        ...
        (__has_type(avro_type) and avro_type.get('type') == ARRAY
            and __is_leaf(avro_type.get('items'))) or

        # Nullable primitive ["null", "aaa"]
        (is_nullable(avro_type) and \
            len([True for avro_child in avro_type if __is_leaf(avro_child)]) == 2)
    )
