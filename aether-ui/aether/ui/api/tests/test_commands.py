...
from ui.management.commands.setup_aether_project import Command
...
class TestSetupAetherProject(TestCase):
    ...
    def test__create_aether_project(self):
        ...
        ui_project_id = Project.objects.first().project_id
        self.assertIsNotNone(ui_project_id)
        aether_project_id = Command().get_aether_project(ui_project_id).json()['id']
        self.assertEqual(str(ui_project_id), aether_project_id)
        self.delete_aether_project(aether_project_id)

    def test__check_existing_aether_project(self):
        setup_aether_project()
        ui_project_id_1 = Project.objects.first().project_id
        aether_projects_count_1 = Command().get_aether_projects().json()['count']
        setup_aether_project()
        ui_project_id_2 = Project.objects.first().project_id
        aether_projects_count_2 = Command().get_aether_projects().json()['count']
        self.assertEqual(ui_project_id_1, ui_project_id_2)
        self.assertEqual(aether_projects_count_1, aether_projects_count_2)
        self.delete_aether_project(ui_project_id_1)
